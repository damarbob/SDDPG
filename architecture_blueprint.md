...
StarDust targets MySQL 8.0.13 or newer (Percona Server 8.0.13+ is supported on the same basis) **or MariaDB 10.11 or newer**. The target engine is detected from the live connection, never configured (ADR `0055`). Older MySQL versions are out of scope: the schema registry's atomicity invariants rely on partial unique indexes, which require the 8.0.13 floor and have no equivalent on any MariaDB version. On MariaDB the identical invariant is instead enforced by a `PERSISTENT` generated column plus a plain `UNIQUE` index (ADR `0054`) — this is exactly the "compatibility shim or generated-column workaround" this section once said was unsupported; that stance is superseded. MariaDB 10.6 and older remains out of scope, for an independent reason: a JSON-column collation divergence below the 10.11 floor has no configuration-only fix (ADR `0054` §Consequences). One behavioral divergence ships accepted rather than engineered around: on MariaDB, range filters and field sorts order supplementary-plane characters at the opposite end from MySQL (ADR `0041`'s Consequences section, ADR `0054` §3). *(Dated note, 2026-09-21: this paragraph originally read MariaDB out of scope entirely and predates ADR 0054/0055; both ADRs and `Support\Dialect` are now the authority on engine support.)* (Operator runbooks that use `EXPLAIN ANALYZE` for cardinality estimation require MySQL 8.0.18+; the engine itself does not call `EXPLAIN ANALYZE` and operates fully on 8.0.13. See ADR `0019`.)

StarDust is a daemonized engine: ingestion and synchronous reads are served by the function API in-process, while four independent background processes — the Watcher, Reconciler, Liberator, and Chronicler (see §2.1) — handle capacity provisioning, queue draining, slot eviction, and asynchronous exports. **Two execution models are supported.** The reference model runs these as supervised long-running processes (e.g., under systemd, supervisor, or a container orchestrator), self-looping their bounded chunk passes internally rather than being invoked discretely per request or per scheduled tick; a supported target for this model MUST provide the ability to run persistent background processes or long-running containers, PHP 8.x with CLI access for the engine's command entry point, and local filesystem write access for export artifacts. The second model — for a host with no persistent-process capability, such as cron-only shared hosting — is the bounded `bin/stardust tick` command; see ADR [`0048`](adrs/0048-bounded-combined-tick-for-cron-driven-hosting.md): one process, one connection, composing the Watcher, Liberator and Reconciler over a fixed time budget, invoked on a schedule (a crontab line or a scheduled URL fetch) rather than run continuously. This mode composes three of the four daemons only — async exports remain unsupported under it until an operator opts into the ADR [`0050`](adrs/0050-chronicler-cooperative-yield-at-a-chunk-boundary.md) cooperative yield via `--exports`. A deployment target that can support neither model is out of scope.
...
- **Safeguards**: Employs Empty-Table-Only DDL, Advisory Locking (`GET_LOCK('stardust_page_provision', 10)`), and Atomic Registry Updates to prevent metadata lock contention. Since ADR [`0053`](adrs/0053-advisory-lock-names-are-qualified-per-installation.md), the literal name reaching the server carries a per-installation suffix derived from the schema name, so two StarDust installations sharing one MySQL server never contend on each other's lock. `ALTER TABLE` on populated pages is strictly forbidden.
...
- **The Liberator is multi-worker, excluded at page-table granularity — neither a strict singleton nor unconstrained multi-worker.** Since ADR [`0049`](adrs/0049-multi-worker-liberator-excluded-per-page.md), any number of Liberator workers may run concurrently, but two workers never sweep the same `entry_slots_page_X` table at the same time: each acquires a page-scoped `GET_LOCK('stardust_sweep_page_{pageId}', 0)` advisory lock before sweeping a slot on that page and skips the slot for the cycle on contention rather than waiting. This is a third coordination category distinct from both of the above — throughput scales with the number of distinct pages holding tombstones, not with worker count alone, and no PID file or process-level lock is involved.
- **Failure Isolation**: Any daemon can crash independently without affecting the others. If the Watcher dies, the Reconciler continues draining into existing pages with remaining capacity. If the Reconciler dies, the Watcher continues provisioning — the queue grows, but writes never block because the exhaustion fallback (above) keeps ingestion operational. If the Liberator dies, dead slots simply stay `tombstoned` and squat on capacity until it recovers. If a Chronicler worker dies mid-export, the abandoned-claim sweep on the next surviving worker re-claims the job (heartbeat lapse) and, per ADR [`0047`](adrs/0047-the-export-resume-anchor-is-the-artifact-plus-its-byte-offset.md), attempts to re-open the prior partial artifact and verify its actual byte length against the stored cursor before resuming — falling back to a fresh artifact from the beginning if that verification fails, rather than trusting the stored `last_cursor` alone; ingestion and synchronous retrieval are never blocked by Chronicler failures because it is a downstream consumer of `entry_data`, not a coordination peer. Ingestion and synchronous retrieval are never blocked by daemon failures.
...
3. **Backfill via Reconciler**: A backfill record is enqueued. The Reconciler processes it in configurable chunks using `JSON_EXTRACT` + type coercion. Entries where the value cannot be coerced (e.g., a non-numeric string being retyped to `int`) store `NULL` in the new slot and fall back to `JSON_EXTRACT` on retrieval; each such NULL emits a `coercion_null` event for operator audit. **Filterability remains suppressed** (slot status = `backfilling`) until backfill completes.
