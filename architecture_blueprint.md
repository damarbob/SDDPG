...
StarDust targets MySQL 8.0.13 or newer (Percona Server 8.0.13+ is supported on the same basis). Older MySQL versions and MariaDB are explicitly out of scope: the schema registry's atomicity invariants rely on partial unique indexes and the registry diagnostic queries rely on common table expressions — both of which require the 8.0.13 floor. Implementations may not target older versions; no compatibility shim or generated-column workaround is supported. (Operator runbooks that use `EXPLAIN ANALYZE` for cardinality estimation require 8.0.18+; the engine itself does not call `EXPLAIN ANALYZE` and operates fully on 8.0.13. See ADR `0019`.)
