...
5. **A page is never provisioned with an empty column list.** Under this decision that is a page with no slot columns and therefore no inventory rows — it contributes nothing to `total_slots`, so the low-capacity ratio that triggered it stays below threshold and the Watcher provisions another one on the next tick, without bound. `ProvisioningPlanner` downgrades such a plan to `no_action`, and `PageProvisioner::provision()` rejects the list independently; the parameter loses its `= []` default so the misuse is a static error.

   This is reachable only from the low-capacity trigger with no demand at `k = 0`, and it **narrows `k = 0` as a way to restore the pre-`0042` policy**: `k = 0` still reproduces the pre-`0042` index set for a demanded family, since the floor of one column is applied by the planner, but for an *undemanded* family the pre-`0042` answer was a page carrying no indexes at all, and under this decision that is a page carrying no columns at all. Such a page is not a degraded version of the old behaviour; it is nothing. The starvation-freedom guarantee is untouched, because the demanded-family floor means a starved family always names at least one column and so can never reach the empty branch.
