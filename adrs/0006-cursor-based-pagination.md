...
> **Extended 2026-08-30 by ADR [`0041`](0041-sort-ordering-and-the-anchored-cursor.md):** the read API now accepts a sort key, which changes two things here. The cursor gains a second wire format — an unsorted read still emits `base64url("v1:" . entryId)` byte-for-byte, while a sorted read emits a `v2:` token carrying the anchor id **plus the sort key identity and direction**. The cursor still carries only an `id`, not the anchor row's sort value: the value is resolved from the anchor row at query time, so "an opaque cursor value derived from the `id` of the last record seen" above stays literally true and a token stays constant-size no matter how wide the sorted field is. And the invalidation rule in this bullet, which had no sort parameter to detect until now, is enforced — a cursor replayed under a different sort key or direction raises a typed error instead of silently walking a different ordering.
