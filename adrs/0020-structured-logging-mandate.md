...
- `chronicler`: `job_claimed`, `job_complete`, `job_failed`, `low_disk`, `artifact_oversized`, `gc_swept`, `artifact_resumed` (ADR `0047` — a re-claimed job's verified re-open of its prior partial artifact, distinct from `job_claimed`'s `claim_kind` because a resume can fail validation and fall back to a fresh artifact even on an abandoned claim).
