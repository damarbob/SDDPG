...
**A note on why this needed a code change rather than an amendment softening the requirement.** The finding was originally recorded as "seven registry sites omit a required field". That was wrong in the engine's favour: `StdoutNdjsonLogger` synthesises a v4 UUID for any event whose context omits one, so no record has ever reached the wire without the field. The defect was never a missing value — it was a *meaningless* one, and it was invisible for exactly that reason. `tests/Smoke/Conventions/EventCorrelationTest` therefore asserts the stricter in-code rule (the id must be **passed**, not merely present), because the wire-level contract this ADR states cannot distinguish the two.

**And that scan is necessary but not sufficient.** A site can pass an id it minted itself and satisfy it completely — which is what both advisory samplers did, inside a retype promotion that already had one. Enforcing the carried-through clause therefore takes two tests: the scan, and a behavioural assertion that the events of one operation share a *value* (`tests/Smoke/LifecycleCorrelationTest`). A new event on any source needs both.

#### Extended to `api`, `bulk_api` and `export_api` (2026-09-06)

The scope note here originally deferred those sources, and undercounted them at three sites. A full enumeration found **twelve**. They are now fixed, on the same rules, with two additions the registry lifecycles did not need.

**A consumer may supply the id.** `EntryPayload`, `EntryQuery`, `ExportJobRequest` and `BulkIngestOptions` each take an optional `correlationId`, and the facade's scalar entry points append the same parameter. Until this, "per-request UUID for API events" described something no consumer could actually do — only `SearchRequest` had the field, and its own docblock scoped it to internal orchestration. A caller behind an HTTP API can now put their request id into the engine's log.

**A bulk id belongs to the call, not the payload.** It lives on `BulkIngestOptions` because the N chunk events of one `bulkWrite()` describe one operation; per-payload ids could not express that.

Three asynchronous handoffs join across a process boundary, and **the join target differs for each** — they are not three copies of the `backfill_checkpoints` pattern:

- **Export** — `stardust_export_jobs.correlation_id`. The Chronicler emits a genuine per-job pair, so `job_claimed` / `job_complete` / `job_failed` take the submission's id directly. Its per-chunk `chunk_written` takes it too, because the Chronicler processes one job across all its chunks in a single continuous call: the job *is* the operation there. That is the opposite of the Reconciler's rule and deliberately so.
- **Import** — `stardust_import_jobs.correlation_id`. This path has **no per-job completion event**, so the chunk events keep their own id and name the submission as `job_correlation_id` alongside. The companion field is the entire mechanism.
- **Sync queue** — `stardust_sync_queue.origin_correlation_id`. **This one cannot join on the success path at all**: a Reconciler chunk claims rows from N unrelated writes, so no chunk event can carry one write's id. The reachable join is failure: the value is copied to `stardust_reconciler_dlq.origin_correlation_id`, beside the existing `chunk_correlation_id`, so a dead letter names both the tick that failed it and the write that created it. Per-row success events were considered and rejected as a volume explosion against ADR `0018`'s chunk-level design.

All four new columns are nullable, and every reader falls back, so they are safe to add under a running fleet. Measured on 8.0.13 across paired interleaved runs, the `stardust_sync_queue` column leaves enqueue cost unchanged (−0.3%, within noise) and ADR `0038`'s `ix_sync_queue_entry` plan identical — checked because that enqueue is on ADR `0007`'s write-availability path.

One thing was deliberately **not** done: adding a per-job completion event to the import path. It would join more cleanly than a companion field, but a new event name means a vocabulary entry here and an `EventVocabularyTest` change, which is its own decision.
...
- `chronicler`: `job_claimed`, `job_complete`, `job_failed`, `low_disk`, `artifact_oversized`, `gc_swept`, `artifact_resumed` (ADR `0047` — a re-claimed job's verified re-open of its prior partial artifact, distinct from `job_claimed`'s `claim_kind` because a resume can fail validation and fall back to a fresh artifact even on an abandoned claim), `job_yielded` (ADR `0050`, added 2026-09-16 — a committed, non-final chunk boundary where the job returned to `pending` with its resume anchor intact rather than continuing; emitted in place of `job_complete`, under the same `correlation_id` as the rest of the job's events since the operation has not ended, only this worker's attempt at it).
...
- `tick` (ADR `0048`, added 2026-09-15): `tick_started`, `tick_complete`, `tick_skipped` (`contended_pid_file` — `watcher`; per the ADR `0049` amendment to Commitment 3, `liberator` can no longer appear here since the Liberator no longer holds a pid-file lock for this run to contend on). The bounded combined-tick run's own trace boundary — one run mints one `correlation_id` and emits `tick_started` / `tick_complete` under it, with `tick_skipped` when the run declines to start at all because another process already holds the Watcher's pid file. Deliberately not a joining id: the Watcher, Liberator and Reconciler it composes each mint their own per-tick ids as they already do under a persistent daemon, so a `tick` run's log groups by timestamp proximity rather than by a shared `correlation_id` — see `docs/observability.md`. Like `source: registry`, there is no `blueprints/` document for this source yet, so the blueprint-update rule below is unsatisfiable for it too — recorded rather than fixed, on the same precedent.
