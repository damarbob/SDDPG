...
- **Singleton.** The Liberator runs as a single instance per deployment, enforced by a PID file or OS-level process lock identical to the Watcher's mechanism (see ADR `0008`). Multiple Liberator workers are explicitly rejected: concurrent `UPDATE ... SET i_str_XX = NULL` statements against the same `(page, slot)` produce no correctness problem (NULL is idempotent), but they multiply InnoDB row-lock contention and obscure sweep progress in the per-slot `sweep_cursor_id`. The sweep workload is dominated by IO and short row locks, not CPU; horizontal scaling delivers no throughput benefit and degrades observability.
  ...
  > **Amended 2026-09-15 by ADR [`0049`](0049-multi-worker-liberator-excluded-per-page.md):** this bullet's rejection was of *concurrent workers sharing the same page*, and remains correct read that way — 0049 does not contradict it. What changes is the unit of exclusion: instead of one process per deployment, one worker at a time is enforced **per `entry_slots_page_N` table** via `GET_LOCK`, so multiple workers now run safely as long as they never share a page. The per-slot `sweep_cursor_id` stays single-writer under this scheme for the same reason it always was — see 0049 Commitment 2.
...
- **Per-batch checkpoint.** The Liberator commits the updated `sweep_cursor_id` (next-batch high-water mark on `entry_id`) to `stardust_slot_assignments` in the **same transaction** as the chunked `UPDATE`. There is no separate "save cursor" step. On crash, the Liberator resumes from the last committed `sweep_cursor_id` for that slot and re-issues the next chunk; already-nullified rows in the resumed chunk are no-ops because `NULL` is idempotent.
  ...
  > **Amended 2026-09-06 by ADR [`0045`](0045-tombstoning-resets-the-sweep-annotations.md):** this ADR specifies the cursor's behaviour *within* one sweep and is silent on what it means when a slot goes round the lifecycle a second time. Nothing reset it, so a recycled column's second sweep resumed from the first occupant's final cursor and completed without nullifying the rows below it — the data-bleeding hazard in this ADR's own Context, arriving through the reuse it declares safe. Measured on 8.0.13: after a promote / demote / promote / demote cycle over twenty entries the reclaimed slot reached `free` holding all twenty of the second occupant's values, with `sweep_chunk` and `sweep_complete` both honest for the range walked. 0045 clears `sweep_cursor_id` and `sweep_gap_count` in the UPDATE that flips a slot to `tombstoned`. **The per-chunk checkpoint above, its crash-resumption property and the deadlock/gap policy below are unchanged** — the reset happens before a sweep begins, never during one.
- **Deadlock policy.** Concurrent reads against the swept page take only shared row-level locks (the bounded fetch in ADR `0005` reads via `WHERE id IN (...)`), so deadlocks against the Liberator's `UPDATE ... LIMIT 500` are uncommon but possible under high read load on the same `tenant_id` partition. On `SQLSTATE 40001` (deadlock detected by InnoDB), the Liberator rolls back the chunk, sleeps for the inter-chunk delay, and retries from the same `sweep_cursor_id` — it does **not** advance the cursor on failure. After three consecutive deadlocks against the same chunk, the Liberator logs the slot identity and skips ahead by `LIMIT` rows, leaving a `sweep_gap` flag on the registry row for operator inspection. This bounded retry prevents a hot read partition from indefinitely blocking sweep progress while still surfacing pathological contention.
  ...
  > **Clarified 2026-09-06 by ADR [`0046`](0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md):** the implementation honoured neither half of this bullet as written, and 0046 restores both rather than changing them. (1) *"Skips ahead by `LIMIT` rows"* had been implemented as `cursor + chunkSize` — a span of **ids**, which under-advances on a sparse page: measured on 8.0.13, crossing one `AUTO_INCREMENT` jump took **101** budget cycles and 101 `sweep_gap_flagged` events where two chunks were actually skipped. The gap now advances by the chunk's own last id. (2) A gapped sweep nonetheless ran on to the final chunk and **reclaimed**, which step 4 above does not permit — reclaim requires the slot be *confirmed 100% nullified*, and this ADR's Consequences call data bleeding "structurally impossible" on that basis. Measured: a slot reached `free` holding ten of the previous field's rows and `SlotReserver` handed that same slot to the next field. A gapped sweep now rewinds to its first gap and stays `tombstoned` for the next cycle to re-walk. **The bounded-retry guarantee is unchanged** — no sweep blocks on a chunk it cannot pass; what changes is that abandoning a chunk defers reclamation instead of completing it.
...
- The singleton model means total sweep throughput is bounded by one process. Operators with extreme tombstone churn must accept longer reclaim latency rather than horizontally scaling the daemon.
  ...
  > **Amended 2026-09-15 by ADR [`0049`](0049-multi-worker-liberator-excluded-per-page.md):** no longer true as written. The Liberator is multi-worker since 0049, bounded instead by the number of distinct pages holding tombstones rather than by process count — an operator with extreme tombstone churn spread across many pages can now run N processes for up to N-way throughput.
...
- Multi-worker Liberator coordinated by `SKIP LOCKED` on `stardust_slot_assignments` — rejected because the sweep workload is IO-bound (bulk `UPDATE` on a single page) rather than CPU-bound, so additional workers do not increase throughput. They do, however, multiply InnoDB row-lock contention against concurrent reads on the swept page and fragment the per-slot `sweep_cursor_id` semantics. The marginal throughput gain does not justify the loss of a clean single-cursor sweep model.
  ...
  > **Amended 2026-09-15 by ADR [`0049`](0049-multi-worker-liberator-excluded-per-page.md):** this bullet rejects `SKIP LOCKED` at **slot** (row) granularity specifically, and 0049 does not reopen that rejection — it still holds for exactly the reasons given here. 0049 instead multi-workers the Liberator at **page-table** granularity via `GET_LOCK`, where the row-lock-contention and cursor-fragmentation objections above do not apply (two workers never touch the same page table concurrently), and the throughput gain is real because it is across distinct IO rather than within one page's.
