...
- **On a MariaDB 10.x target (see [`0054`](0054-mariadb-10-11-support-floor.md)), range operators and field sorts order supplementary-plane characters at the opposite end from a MySQL target.** MySQL's `utf8mb4_0900_ai_ci` sorts them before all BMP text; the best available MariaDB 10.x collation (`utf8mb4_unicode_520_nopad_ci`, UCA 5.20) sorts them after. This is **not** a tie-order difference the mandatory `entry_data.id` tiebreak can rescue — the two engines disagree on the relative order of genuinely unequal values, not merely on ties within one equivalence class. It reaches exactly the range operators (`lt`, `lte`, `gt`, `gte`, `between` when the range spans the boundary) and a `Field` sort; every equality-family operator, and every deployment whose data contains no supplementary-plane characters, is unaffected. On a MySQL-only deployment this ADR is unchanged and this note does not apply.
- **Correction, 2026-09-20.** `src/Read/CLAUDE.md`'s "string slots sort exactly, on the full value" claim — the load-bearing premise this ADR's field-sort design rests on — was verified only against MySQL 8.0.13, where `max_sort_length` genuinely does not truncate `ORDER BY` correctness even forced down to 8. **MariaDB does not share this exemption.** Measured directly in SQL against real 10.6/10.11/11 servers: rows sharing an identical prefix of `max_sort_length` bytes (default 1024, same nominal value on both engines) come back in scan order rather than sort order, and the effect is collation-independent (reproduced with both `utf8mb4_unicode_520_nopad_ci` and `utf8mb4_general_ci`). Because the keyset pagination predicate compares the *full* value regardless, a truncated `ORDER BY` that disagrees with it can skip or repeat rows across a page boundary — silently, the same failure class the three-branch NULL-handling predicate exists to avoid. **Closed as an engine-selection consequence, not a design change**: `Search\Mysql\MysqlNativeDriver` now raises `max_sort_length` to cover the full string-slot bound (`FilterLimits::DEFAULT_MAX_STRING_LENGTH × 4` bytes) on a MariaDB target only, leaving MySQL's resource profile untouched since its correctness never depended on the setting. This ADR's field-sort design and query shape are unaffected; only the session tuning needed to make MariaDB honour what MySQL already guaranteed.
