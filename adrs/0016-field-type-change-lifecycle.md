...
**Filterability is held for the entire backfill window.** The field's slot status is set to `retyping` at sever time and advances to `backfilling` once the new slot is assigned. In both states, the `is_filterable` flag has no effect on query routing — all reads fall back to `JSON_EXTRACT`. Filterability is restored only when the Reconciler advances the slot status to `ready`. This prevents filtered queries from silently returning incomplete results during the migration window.
