...
>
> **Refined 2026-08-10 by ADR [`0035`](0035-usable-capacity-is-a-satisfiability-test.md):** reading (2) above specifies a usable-capacity *numerator* but no denominator, and both candidate denominators make the threshold comparison diverge — the daemon provisions a page per tick, or cascades several pages for a single waiting field. ADR `0035` supersedes the ratio reading: usable capacity governs provisioning as a **per-family satisfiability test** (a demanded family with zero claimable free slots provisions unconditionally), OR-composed with the existing global-ratio headroom check. The guarantee stated here is unchanged and in fact strengthened — see ADR `0035` for the divergence proofs and the index-set sizing rule.
