...
- The lease-loss detector's justification needs care when read. The checkpoint UPDATE carries `WHERE id = ? AND worker_identity = self`, and `rowCount() === 0` is taken to mean "a re-claimer overwrote our identity, never a no-op update". That inference rests on `entries_written` being monotonic, which is true by construction. The appended record also makes the row differ, but it is not what the guarantee rests on and must not be relied on for it.
  ...
  **Dated addendum, 2026-09-21.** `rowCount() === 0` is not the only shape this failure takes. Verified against a real MariaDB 11 container: the checkpoint UPDATE can throw SQLSTATE HY000 errno 1020 ("Record has changed since last read in table") instead of matching zero rows, under the identical race this bullet describes. Root cause: `manifest` is a JSON column, and MariaDB attaches an implicit `CHECK (json_valid(manifest))` to every JSON column; that constraint's row re-check collides with InnoDB's semi-consistent read for this statement's `WHERE` clause when a sibling connection changes the row between read and write. MySQL and MariaDB 10.11 never raise it for this statement — reproduced 3/3 on 11, 0/3 on the other two — and the Chronicler's equivalent checkpoints (`ExportJobProcessor`), which update no JSON column, were checked directly and do not hit it either. `ImportJobWorkSource::isRowChangedSinceLastRead()` gives errno 1020 the identical lease-lost treatment as `rowCount() === 0` rather than letting it fall through to `failJob()`. This does not change the detector's design — both signals mean the same thing — only its implementation on one engine.
