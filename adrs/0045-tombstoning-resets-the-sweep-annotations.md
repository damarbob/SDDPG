...
- **It does not close the gap path, which is a separate defect and not a shortfall of this decision.** *(Amended 2026-09-06: this bullet originally called the gap path "a separate accepted risk in ADR `0009`". That was wrong — `0009` step 4 permits reclaim only on a slot confirmed 100% nullified, so a gapped sweep reclaiming was the implementation diverging from the ADR, not the ADR accepting anything. Closed by ADR [`0046`](0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md).)* After `deadlockRetryBudget` consecutive lock failures the sweep abandons a chunk, advances past it, and still reaches `sweep_complete` — so the slot reaches `free` with the abandoned range still populated. Measured on 8.0.13 (twenty rows, `chunkSize = 10`, one gap): the slot reclaims to `free` holding **ten** of the previous occupant's values, and `SlotReserver` then hands that same slot to the next field, which takes it `assigned` with the ten rows still there. `LiberatorDeadlockRetryTest::testThreeConsecutiveDeadlocksTriggersSweepGap` has asserted the residue half of that since Phase 6a. The blueprint justified accepting a gap on the grounds that "the field's data is still authoritative in `entry_data.fields`" — which answers whether the *departing* field loses data, not whether the *arriving* one inherits it. ADR `0009` step 4 answers the second question and forbids it; ADR `0046` restores that.
...
- ADR [`0046`](0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md) — closes the gap path, the remaining route to the same residue, by restoring ADR `0009` step 4. It builds directly on this ADR: a cursor of `NULL`/`0` meaning "start from the beginning" is what lets a gapped sweep rewind in one clause.
