...
> **Extended 2026-08-28 by ADR [`0040`](0040-import-manifest-enumerates-chunks.md):** §26 above is unchanged in intent, and the **synchronous** path implements it as written. The **asynchronous** path shipped with a `{chunks, entries_written}` resume checkpoint instead of a manifest, and 0040 is the change that makes it enumerate chunks with their entity ID ranges. Two clarifications 0040 adds, which cannot be read out of the text above: the `rolled_back` outcome is **synchronous-only** — a failed chunk is terminal for an async job, per §38 — and a job that fails before producing any chunk (a `malformed_json` artifact failure) carries **no manifest at all**, which is what "for jobs that have produced any chunks" means in practice.
