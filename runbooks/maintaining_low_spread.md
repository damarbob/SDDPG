...
| A tombstone that will not drain | The slot's sweep keeps abandoning a chunk under lock contention, and per [ADR 0046](../adrs/0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md) a gapped sweep **defers** reclamation rather than freeing a slot it did not empty — so the capacity stays squatted until a pass completes clean. This is working as designed and fails safe; the alternative was handing the next field a column with the previous one's data in it. **Query on age, not on `sweep_gap_count`** — `SELECT id, page_id, slot_column, tombstoned_at, sweep_gap_count, sweep_cursor_id FROM stardust_slot_assignments WHERE status = 'tombstoned' ORDER BY tombstoned_at`. A slot whose sweep cannot commit *anything* carries `sweep_gap_count = 0` and a NULL cursor, because the counter rides the next successful chunk commit (ADR 0046) and there has not been one; measured, that is exactly what sustained contention on the registry row produces. A non-zero counter means the sweep is making progress and skipping chunks, which is the milder case. Age catches both and reduce write/read pressure on it, or lower `liberatorChunkSize` so each chunk takes fewer row locks. `sweep_gap_flagged` names the exact id range that keeps failing. **Watch queue depth too**: the Liberator sweeps oldest-tombstone-first, so a stuck slot keeps its place at the head of every batch and enough of them can starve newer tombstones — raise `liberatorBatchSize` if the stuck head is consuming the cycle. |
