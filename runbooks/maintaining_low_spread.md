...
| A tombstone that will not drain, with a climbing `sweep_gap_count` | The slot's sweep keeps abandoning a chunk under lock contention, and per [ADR 0046](../adrs/0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md) a gapped sweep **defers** reclamation rather than freeing a slot it did not empty — so the capacity stays squatted until a pass completes clean. This is working as designed and fails safe; the alternative was handing the next field a column with the previous one's data in it. Find the contended page (`SELECT id, page_id, slot_column, sweep_gap_count, sweep_cursor_id FROM stardust_slot_assignments WHERE status = 'tombstoned' AND sweep_gap_count > 0`) and reduce write/read pressure on it, or lower `liberatorChunkSize` so each chunk takes fewer row locks. `sweep_gap_flagged` names the exact id range that keeps failing. **Watch queue depth too**: the Liberator sweeps oldest-tombstone-first, so a stuck slot keeps its place at the head of every batch and enough of them can starve newer tombstones — raise `liberatorBatchSize` if the stuck head is consuming the cycle. |
