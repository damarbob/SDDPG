...
| `failed_reason`   | `VARCHAR(64)` **NULL**                              | Closed taxonomy: `artifact_size_exceeded`, `query_failure`, `disk_full`, `other`. Set only when `status='failed'`.                            |
| `worker_identity` | `VARCHAR(128)` **NULL**                             | Hostname/PID of the claiming Chronicler worker. Audit and observability.                                                                      |
