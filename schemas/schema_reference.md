...
3. **Operational & Coordination** (§5) — `stardust_schema_version`, `stardust_export_jobs`, `stardust_reconciler_dlq`, `backfill_checkpoints`, `stardust_import_jobs`, `stardust_advisory_schedule`. Tables that exist for daemon coordination, async work, operator triage, and migration state.
...
| `origin_correlation_id` | `VARCHAR(36)` **NULL** | Per ADR [`0020`](../adrs/0020-structured-logging-mandate.md): the id of the write that enqueued this row. **Cannot join on the success path** — a Reconciler chunk claims rows from N unrelated writes, so no chunk event can carry one write's id. The reachable join is failure: on quarantine, this value is copied to `stardust_reconciler_dlq.origin_correlation_id` beside that table's own `chunk_correlation_id`, naming both the write that created the row and the tick that failed it. |
...
| `previous_name` | `VARCHAR(128)` **NULL**                     | Bridges an ADR [`0036`](../adrs/0036-entry-payload-keys-are-field-names.md) rename window. `entry_data.fields` is keyed by field name, so a rename flips `name` immediately and rewrites payloads asynchronously; a non-null value means exactly "a rename is in flight for this field", and both the read-path slot resolver and the write-path live-slot map fall back to it. Cleared in the same transaction that completes the backfill, alongside the schema-version bump. |
| `deleted_at`    | `DATETIME` **NULL**                         | Bridges an ADR [`0037`](../adrs/0037-field-deletion-lifecycle.md) deletion window, deliberately the same shape as `previous_name` above. A non-null value means exactly "a deletion is in flight for this field", and every registry reader carries `deleted_at IS NULL` from that moment. Not a soft-delete tier — there is no undelete, and the purge's final chunk hard-deletes the row. |
...
| `sweep_cursor_id` | `BIGINT` **NULL**                                            | The Liberator's per-slot `entry_id` cursor for the chunked nullification UPDATE. Per-*sweep*, not per-column: cleared by the tombstone (§4.6.1), advanced by the sweep, preserved by the reclaim. ADR [`0045`](../adrs/0045-tombstoning-resets-the-sweep-annotations.md). |
| `sweep_gap_count` | `INT` NOT NULL DEFAULT `0`                                   | Count of chunks the current sweep abandoned after exhausting its lock-retry budget (ADR [`0009`](../adrs/0009-tombstone-based-slot-eviction.md) gap path). Same lifecycle as `sweep_cursor_id`. **A non-zero value on a `tombstoned` slot means that slot is deferring reclamation until a pass completes without a gap** (ADR [`0046`](../adrs/0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md)); a climbing value is the contention signal. On a `free` or live slot it is the historical record of the sweep that emptied the column — not a residue warning, since a gapped sweep never reaches `free`. |
...
1. **Sever + tombstone** — `status: assigned → tombstoned`, `field_id → NULL`, and `sweep_cursor_id → NULL` / `sweep_gap_count → 0` in one commit. The annotation reset rides here rather than on the reclaim or on the next reservation, so a recycled slot's sweep starts at the beginning of the page instead of resuming from its previous occupant's cursor — ADR [`0045`](../adrs/0045-tombstoning-resets-the-sweep-annotations.md), which amends ADR [`0009`](../adrs/0009-tombstone-based-slot-eviction.md). `field_id` is nulled **before** the status flip, never in one UPDATE; every later reference to a "standard sever" means this whole tuple.
...
3. **Sweep completion** — the Liberator's final `UPDATE entry_slots_page_X SET i_str_XX = NULL WHERE id > ? ...` batch (keyed on `(page, slot_column)` for `id > sweep_cursor_id`, no `tenant_id` predicate — ADR [`0029`](../adrs/0029-liberator-sweep-omits-tenant-predicate.md)) and the registry's `status: tombstoned → free` (plus `field_id → NULL`) commit together. **The reclaim half is conditional**: per ADR [`0046`](../adrs/0046-a-gapped-sweep-skips-the-chunk-and-does-not-reclaim.md) a sweep that abandoned any chunk commits the final batch but keeps `status = 'tombstoned'` and rewinds `sweep_cursor_id` to its first gap, in that same transaction, so §4.1's reclaim precondition — the slot is confirmed 100% nullified — holds without exception. `sweep_cursor_id` and `sweep_gap_count` are deliberately **not** reset here — they stay readable on the reclaimed slot as the record of the sweep that emptied it, and are cleared by the next sever (§4.6.1).
...
| `correlation_id`  | `VARCHAR(36)` **NULL**                              | Per ADR [`0020`](../adrs/0020-structured-logging-mandate.md): the submission's operation id, taken directly by `job_claimed` / `job_complete` / `job_failed` and by the per-chunk `chunk_written` — the Chronicler processes one job across all its chunks in a single continuous call, so the job *is* the operation. Nullable because a job already in flight when the column lands has no id; every reader falls back to a per-event id in that case. |
...
| `artifact_path`   | `VARCHAR(512)` **NULL**                             | Absolute filesystem path of the materialized file. Per ADR [`0047`](../adrs/0047-the-export-resume-anchor-is-the-artifact-plus-its-byte-offset.md), written on **every** chunk commit (not only the final one), so a resuming worker has an anchor to verify — not merely `NULL` until `status='completed'`. Nulled by `markFailed()` (alongside `artifact_bytes`, below) on terminal failure, and by the GC sweep after TTL.                                                                                                          |
| `artifact_bytes`  | `BIGINT` **NULL**                                   | The artifact file's verified byte length as of the last committed chunk, per ADR [`0047`](../adrs/0047-the-export-resume-anchor-is-the-artifact-plus-its-byte-offset.md) Commitment 1. Written alongside `artifact_path` on every chunk commit; a resume re-opens the file, verifies it holds at least this many bytes, and truncates/seeks to this offset before continuing — replacing a bare trust of `last_cursor`. Nulled by `markFailed()` alongside `artifact_path` so a `failed` row never advertises an anchor to bytes that no longer exist. |
...
| `origin_correlation_id` | `VARCHAR(36)` **NULL** | Per ADR [`0020`](../adrs/0020-structured-logging-mandate.md): copied from `stardust_sync_queue.origin_correlation_id` at quarantine time — the id of the write that originally enqueued the row, distinct from `chunk_correlation_id` above (the tick that failed it). This is the one reachable join for a sync-queue row's origin, since a successful drain never emits a per-row event to join on. |
...
| `correlation_id`       | `VARCHAR(36)` **NULL**                       | Per ADR [`0020`](../adrs/0020-structured-logging-mandate.md): carries the asynchronous lifecycle's operation id across the process boundary, so `rename_complete` / `promote_to_ready` / `delete_complete` / `model_delete_complete` are emitted under the same id their `*_started` half carried. Reset in every `INSERT ... ON DUPLICATE KEY UPDATE`, for the same reason as `source_declared_type` — a second lifecycle reusing the row must not inherit the first one's id. Nullable because a checkpoint already `running` when the column landed has no id, and every reader falls back to the per-chunk id (`chunk_correlation_id`, reported alongside it in the log, not a column here) for that row. |
...
| `correlation_id`  | `VARCHAR(36)` **NULL**                              | Per ADR [`0020`](../adrs/0020-structured-logging-mandate.md): the submission's operation id. This path has **no per-job completion event**, so the Reconciler's chunk events keep their own id and name this one as `job_correlation_id` alongside — the companion field is the entire join mechanism. Nullable for the same reason as `stardust_export_jobs.correlation_id`. |
