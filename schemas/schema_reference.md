...
| `sweep_gap_count` | `INT` NOT NULL DEFAULT `0`                                   | Count of chunks the current sweep abandoned after exhausting its lock-retry budget (ADR [`0009`](../adrs/0009-tombstone-based-slot-eviction.md) gap path). Same lifecycle as `sweep_cursor_id`. **A non-zero value on a `free` or live slot means the column may still hold residue from the sweep that preceded its current contents.** |
