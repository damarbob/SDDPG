...
2. Provisioning acquires an advisory lock (`GET_LOCK('stardust_page_provision', 10)`) and releases it upon completion or failure. **Since [ADR 0053](../adrs/0053-advisory-lock-names-are-qualified-per-installation.md)**, the literal name reaching the server carries a per-installation suffix derived from the schema name, so two StarDust installations sharing one MySQL server never contend on each other's Watcher; the base name and the 10-second timeout stated here remain normative and unchanged.
...
6. Each poll cycle logs: timestamp, pages inspected, usable capacity percentage, action taken (provisioned / no action), and — when a page is provisioned — the indexed slot columns emitted for it and the pending demand that drove them. **Since [ADR 0052](../adrs/0052-the-advisory-schedule-is-persisted-and-fleet-wide.md)**, `poll_complete` also carries `advisories_sampled` (boolean): the advisory schedule is fleet-wide, claimed by a conditional UPDATE against the shared `stardust_advisory_schedule` singleton, so a cycle can find the sample due but lose the claim to another host. `advisories_sampled` is what distinguishes that outcome ("due, but another host won the claim") from "not due at all" — both look identical otherwise, since neither runs the sampler.
