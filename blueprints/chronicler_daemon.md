...
1. After a committed, **non-final** chunk, the processor consults an optional yield signal. A non-null cause folds into the SAME chunk-commit transaction: `status='pending', worker_identity=NULL`, with `last_cursor`/`heartbeat_at`/`skip_count`/`artifact_path`/`artifact_bytes` written exactly as any other chunk commit and `claimed_at`/`correlation_id` left untouched. The processor returns without reaching a terminal state; the caller sees this as a distinct, non-terminal `JobOutcome::Yielded`.
2. A yield is never offered before the first committed chunk and never on the final chunk — a yield always carries at least one chunk of progress, and the last chunk always completes the job regardless of what the signal would report.
3. The artifact's byte anchor is captured immediately after `flush()`, BEFORE the stream is closed. `close()` then runs BEFORE the commit, releasing the exclusive file lock before the row becomes claimable — the inverse of the abandoned-claim path, where a hung prior worker may still hold it. A resumer of a genuinely yielded job therefore never observes `restart_cause: 'locked'`.
4. The yield-commit UPDATE reuses the ordinary chunk commit's `WHERE id=? AND worker_identity=?` lease-loss detector unmodified. A yield racing a genuine concurrent re-claim resolves exactly as an ordinary chunk commit racing one does: `rowCount()==0` ⇒ `lease_lost`, the already-closed artifact is left on disk, terminal-state ownership transfers to the re-claimer.
5. `ExportJobClaimer`'s pending-job claim path (not only the abandoned-claim path) now selects and forwards `last_cursor`/`artifact_path`/`artifact_bytes`. A `pending` row carrying a usable anchor (non-null path, positive byte count — the same predicate the artifact-stream factory applies before attempting adoption) is a previously-yielded job, not a fresh submission, and claims with `claim_kind: 'resumed'` rather than `'pending'`.
6. `bin/stardust tick --exports` (ADR 0048) composes this daemon last in each round, passing one per-run yield signal (a budget deadline plus the run's own shutdown probe) to every round. The persistent `bin/stardust chronicler` daemon separately composes its own shutdown signal as its default yield signal, so a `SIGTERM` mid-export yields at the next chunk boundary instead of blocking until the job finishes.
...
| :--------------------- | :------ | :---------------------------------------------------------------------------------------------------------------------------------- |
| `job_claimed`          | `info`  | `job_id`, `worker_identity`, `claimed_at`, `claim_kind` (`pending` \| `abandoned` \| `resumed` — ADR 0050: a `pending` row carrying a usable resume anchor, i.e. a previously-yielded job rather than a fresh submission). |
...
- `job_yielded` shares its `correlation_id` with the rest of the job's events, exactly as `job_complete` does — a yield does not end the operation, only this worker's attempt at it. Unlike `job_complete`, its `rows_streamed_total` is NOT cumulative across the job's whole lifetime: a job yielded three times and re-claimed each time emits four separate row counts (three `job_yielded` plus one final `job_complete`), each scoped to the worker attempt that produced it. An operator computing total rows exported sums across all four.
...
- Cooperative yield at a chunk boundary, extending the resume anchor to the pending-claim path: [ADR 0050](../adrs/0050-chronicler-cooperative-yield-at-a-chunk-boundary.md).
